...
{
    public class CompClockSVG : ComponentBase
    {
        [Inject]
        ...
        protected override void OnAfterRender(bool firstRender)
        {
            if (ClockSettings.FastMode)
            {
                ...
                BCSCJsInterop.Run(new TransferParameters(ClockSettings.timerInterval, ClockSettings.FastMode_Increment), jsRuntime);
            }
            else
            {
                BCSCJsInterop.Run(new TransferParameters(ClockSettings.timerInterval, 0), jsRuntime);
            }
